...
const FiatToCoin = () => {
	...
	const handleConversion = () => {
		...
		const calculatedValue = (parseFloat(fiatValue) / rate).toFixed(6);
		setCoinValue(calculatedValue);
	};
	...
	return (
		<>
			<div
				...
				style={{ backgroundImage: "url(src/assets/GlassMorphBgFiat.png)" }}
			>
				<HeroSection page={"Fiat"} />
				<div
				...
				>
					...
					<div className="flex items-center bg-slate-100 p-6 mb-2 rounded-[20px]">
						<div className="flex flex-col w-full mr-4">
							...
							<input
								...
								value={coinValue}
								className="bg-transparent text-4xl font-medium outline-none w-full py-1"
								placeholder="Converted coin value"
								style={{ color: "#1E1E1E" }}
							/>
						</div>
						<select
					...
					<button
						...
						style={{
							backgroundColor: fiatValue > 0 ? "#308EFF" : "#CDE1EE",
							color: fiatValue > 0 ? "#FFFFFF" : "#87A2B9",
						}}
						disabled={fiatValue <= 0}
					>
						Convert
					</button>
				</div>
